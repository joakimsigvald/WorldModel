...
With the emergence of the couple on level-1, there is in effect a vertical bond between each of the bits and the compound they are now constituents of. This make their horizontal bond redundant and it dissolves. This phenomenon is important, because it frees up the bits connection ports to make new external connections. Thus, the couple has in effect two connectors and can form chains with other couples.

It is important to note that the only thing the couple ever "see" of the world outside of itself is its two bits. It can see and influence its bits states through the vertical bonds. So it is a (useful) simplification to say that the couple has two connector ports and can connect with other couples; it is only bits that can connect with other bits. But the couple can sense those connections through its bits states and it can influence its bits to make or break connections.
...
A ring imposes stronger synchronization pressures on its bits and couples. The entire structure may stabilize into a higher-order entity: a **loop**.

Before a ring elevates into a loop, its bits must not only share a common update frequency but also adopt phase offsets that remain consistent around the closed cycle. This cyclic constraint produces a coherent phase pattern along the ring.

When the loop emerges—with vertical bonds to its constituent couples—the horizontal bonds that previously held the ring together become redundant and dissolve. But the cyclic pattern of phase offsets remains.

This pattern introduces the first topological form of directionality in the system: a loop may wind its phase either clockwise or counter-clockwise. This is the model’s first appearance of **chirality**.

A loop, like a couple, is not merely a ring-shaped structure but a new unified entity with its own internal time and state. It is a Level-2 subject: its constituent couples are vertically bound to it, and their previous horizontal bonds dissolve. Although only bits connect directly at the fundamental level, we may speak of loops connecting to other loops whenever their constituent bits form bonds. This abstraction will be used throughout the rest of the book.
